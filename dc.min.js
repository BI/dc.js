...
}function f(b){a.event.defaultPrevented||j.onClick(b)}function g(a,b){function c(a,b){var c=b;i(a,c)||j.nodes.push({name:a[c],columnName:c})}function d(c,d,f){var g=d;if(f<a.length-1){var h=a[f+1].columnName;e({name:c[g],column:g},{name:c[h],column:h},c[b])}}function e(a,b,c){var d=f(a,b),e=Number(c);d?d.value=d.value+e:j.links.push(g(a,b,e))}function f(a,b){for(var c=h(a),d=h(b),e=j.links.length,f=0;e>f;f++){var g=j.links[f];if(g.source===c&&g.target===d)return g}return!1}function g(a,b,c){return{source:h(a),target:h(b),value:c}}function h(a){for(var b=j.nodes.length,c=0;b>c;c++){var d=j.nodes[c];if(d.name===a.name&&d.columnName===a.column)return c}return-1}function i(a,b){return j.nodes.some(function(c){return c.name===a[b]&&c.columnName===b})}var j={nodes:[],links:[]},k=!1;if(a.forEach(function(a){var d=a.columnName,e=a.dimension.top(1/0);return e.length?void e.forEach(function(a){a[b]>0&&c(a,d)}):void(k=!0)}),k)return null;var l=!0;return a.forEach(function(a,c){var e=a.columnName,f=a.dimension.top(1/0);f.forEach(function(a){a[b]>0&&(l=!1,d(a,e,c))})}),l===!0?-1:j}var h,i,j=b.hierarchyMixin(b.baseMixin({})),k={top:1,right:1,bottom:6,left:1},l=960-k.left-k.right,m=500-k.top-k.bottom,n="No data for the selected filters",o="All data found was negative values.",p=a.format(",.0f"),q=function(a){return p(a)},r=(a.scale.category20(),function(a){return a.source.name+" → "+a.target.name+"\n"+q(a.value)}),s=function(a){return a.name+"\n"+q(a.value)},t=function(a){return a.name};return j.label=function(a){return arguments.length?(t=a,j):t},j.noDataMessage=function(a){return arguments.length?(n=a,j):n},j.negativeDataMessage=function(a){return arguments.length?(o=a,j):o},j.nodeToolTip=function(a){return arguments.length?(s=a,j):s},j.linkToolTip=function(a){return arguments.length?(r=a,j):r},j.transitionDuration(450),j.width=function(a){return arguments.length?(l=a,j):l},j.height=function(a){return arguments.length?(m=a,j):m},j.initData=function(){if(!j.levels()||!j.measureColumn())throw"Must provide dimension column array levels and the measureColumn";return i=g(j.levels(),j.measureColumn()),null===i?null:-1===i?-1:j},j._doRender=function(){function c(){}function d(b){a.select(this).attr("transform","translate("+b.x+","+(b.y=Math.max(0,Math.min(m-b.dy,a.event.y)))+")"),h.relayout(),v.attr("d",u)}function g(){}var p=j.initData();if(null===p)return j.root().html(""),j.root().append("div").classed("no-data-sankey",!0).text(n),p;if(-1==p)return j.root().html(""),j.root().append("div").classed("sankey-negative-data",!0).text(o),p;j.root().classed("dc-sankey",!0),j.root().classed("dc-chart",!1),j.resetSvg();var q=j.svg().attr("width",l+k.left+k.right).attr("height",m+k.top+k.bottom).append("g").attr("transform","translate("+k.left+","+k.top+")");h=a.sankey().nodeWidth(15).nodePadding(10).size([l,m]);var u=h.link();h.nodes(i.nodes).links(i.links).layout(32);var v=q.append("g").selectAll(".link").data(i.links).enter().append("path").attr("class","link").attr("d",u).style("stroke-width",function(a){return Math.max(1,a.dy)}).sort(function(a,b){return b.dy-a.dy});v.append("title").text(r);var w=q.append("g").selectAll(".node").data(i.nodes).enter().append("g").attr("class",function(a){return"node "+b.utils.nameToId(a.name)}).attr("transform",function(a){return"translate("+a.x+","+a.y+")"}).on("click",f).call(a.behavior.drag().origin(function(a){return a}).on("dragstart",c).on("drag",d).on("dragend",g)).classed("deselected",function(a){return j.hasFilter()?!e(a):!1}).classed("selected",function(a){return j.hasFilter()?e(a):!1});w.append("rect").attr("height",function(a){return a.dy}).attr("width",h.nodeWidth()).attr("class",function(a){return"node-"+a.columnName}).append("title").text(s),w.append("text").attr("x",-6).attr("y",function(a){return a.dy/2}).attr("dy",".35em").attr("text-anchor","end").attr("transform",null).text(t).filter(function(a){return a.x<l/2}).attr("x",6+h.nodeWidth()).attr("text-anchor","start"),j.levels().forEach(function(a){j.select(".filter.column_"+a.columnName).html(""),j.hasFilter(a.columnName)&&j.select(".filter.column_"+a.columnName).append("span").classed("reset",!0).text(j.filters(a.columnName)[0]).on("click",function(){j.filterAll(a.columnName),j.redraw()})})},j.onClick=function(a){var c=a.name;b.events.trigger(function(){j.filter(a.columnName,c),j.redrawGroup()})},j._doRedraw=function(){return j._doRender()},j.anchor(c,d)},b.abstractBubbleChart=b.bubbleMixin,b.baseChart=b.baseMixin,b.capped=b.capMixin,b.colorChart=b.colorMixin,b.coordinateGridChart=b.coordinateGridMixin,b.marginable=b.marginMixin,b.stackableChart=b.stackMixin,b}"function"==typeof define&&define.amd?define(["d3"],a):"object"==typeof module&&module.exports?module.exports=a(d3):this.dc=a(d3)}();
